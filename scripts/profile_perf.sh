...
#
# Usage:
#   scripts/profile_perf.sh           # default m=2048, iters=2
#   scripts/profile_perf.sh 1024 4
#
# Notes for WSL2:
#   - perf must be installed (sudo apt install linux-tools-generic or
#     linux-tools-$(uname -r) on a real Linux).
...
if ! command -v perf >/dev/null 2>&1; then
    ...
    echo "  On Ubuntu/WSL2: sudo apt install linux-tools-generic linux-tools-$(uname -r 2>/dev/null) || true" >&2
    exit 1
fi
