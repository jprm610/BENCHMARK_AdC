# Especificacion de la API del benchmark (`main_server`)
...
Este documento describe el contrato publico de las funciones expuestas en `src/` para la rama `main_server`. Las firmas estan en C, nombres y comentarios en ingles. La organizacion del documento sigue las capas del proyecto: `core`, `microkernels`, `algorithms`, `drivers`, `tests`.

El estado consolidado por fase del proyecto vive en la seccion 9 del [`README.md`](../README.md). Aqui solo se documenta el contrato API.
...
3. [Capa `microkernels`](#3-capa-microkernels)
   - 3.1 [`kernel_avx512_morton`](#31-kernel_avx512_morton)
   - 3.2 [`kernel_avx512_tiled`](#32-kernel_avx512_tiled)
4. [Capa `algorithms`](#4-capa-algorithms)
   ...
   - 4.4 [`matmul_tiled_ikj_avx512`](#44-matmul_tiled_ikj_avx512)
   - 4.5 [`matmul_tiled_ikj_omp`](#45-matmul_tiled_ikj_omp)
   - 4.6 [`matmul_morton`](#46-matmul_morton)
   - 4.7 [`matmul_morton_avx512`](#47-matmul_morton_avx512)
...
con $C$ de salida ($m \times n$, sobrescrito), $A$ de entrada ($m \times k$), $B$ de entrada ($k \times n$), sin aliasing entre los tres. Las variantes que requieren $A$ en un layout distinto (Morton) reciben el buffer ya convertido pero conservan la misma firma.
...
Microkernels AVX-512 + FMA, header-only `static inline`. Ningun `.o` separado: el cuerpo se inline en cada TU que los incluye y se compila con `-O3 -march=native` (el `-march=native` en el EPYC 9R45 activa AVX-512F/VL/BW/DQ/IFMA, BMI2 y FMA).

Cada microkernel tiene la responsabilidad de mantener su tile de $C$ en registros ZMM durante toda la pasada $k_c$; el caller garantiza la geometria $(M_R \times N_R)$ y `restrict` no-aliasing. Los $32$ registros ZMM arquitecturales del Zen 5 ($2 \times$ los YMM del Zen 2) permiten mantener tiles mas grandes en registros sin spilling.

### 3.1 `kernel_avx512_morton`

**Archivo:** [`src/microkernels/kernel_avx512_morton.h`](../src/microkernels/kernel_avx512_morton.h).

Microkernel $4 \times 32$ usado por la familia Morton-de-bloques (Secciones 4.7 y 4.8). Los $128$ elementos del tile $C$ caben en $8$ registros ZMM (4 filas $\times$ 2 vectores de 16 lanes FP32); $4$ ZMM extra para los broadcasts de $A$ y $2$ para $B$ totalizan $14$ de $32$ ZMMs vivos en estado estable.

#### Geometria

```c
#define KERNEL_AVX512_MORTON_MR  4u    /* filas por tile */
#define KERNEL_AVX512_MORTON_NR 32u    /* columnas por tile */
```
...
**Computa** $C \mathrel{+}= A \cdot B$ sobre el tile $4 \times 32$ (**acumulacion**, no asignacion). Caller debe zerar $C$ antes si necesita reset.

**Precondiciones:**
- `kc >= 1`, `lda >= kc`, `ldb >= 32`, `ldc >= 32`.
- `C`, `A`, `B` no aliasan (`restrict`).
- Alineacion preferida pero no obligatoria (usa `_mm512_loadu_ps`/`_mm512_storeu_ps`).

**Postcondiciones:**
- $C[r, c] \mathrel{+}= \sum_{p=0}^{kc-1} A[r, p] \cdot B[p, c]$ para $r \in [0, 4)$, $c \in [0, 32)$.
- $A$ y $B$ no se modifican.

### 3.2 `kernel_avx512_tiled`

**Archivo:** [`src/microkernels/kernel_avx512_tiled.h`](../src/microkernels/kernel_avx512_tiled.h).

Microkernel BLIS-style $6 \times 32$ usado por la familia `tiled_ikj` (Secciones 4.4 y 4.5). Los $192$ elementos del tile $C$ caben en $12$ registros ZMM (6 filas $\times$ 2 vectores de 16 lanes); $2$ ZMM para $B$ y $1$ ZMM rotativo para los broadcasts de $A$ totalizan $15$ de $32$ ZMMs vivos.

#### Geometria

```c
#define KERNEL_AVX512_TILED_MR  6u    /* filas por tile */
#define KERNEL_AVX512_TILED_NR 32u    /* columnas por tile */
```

Estas constantes coexisten con las del modulo de algoritmo (`TILED_IKJ_AVX512_MR`, `TILED_IKJ_OMP_MR`, etc.) que son numericamente iguales. La duplicacion es deliberada: el microkernel publica las suyas en el header del propio microkernel, y cada modulo de algoritmo publica las suyas en su propio header para que sus callers no tengan que incluir el del microkernel.
...
**Computa** $C \mathrel{+}= A \cdot B$ sobre el tile $6 \times 32$ (**acumulacion**). Mismo contrato de pre/postcondiciones que `kernel_avx512_4x32`, escalado a $M_R = 6$, $N_R = 32$.

#### `kernel_avx512_tiled_residual_rows`

Fallback AVX-512 para las colas $m \bmod M_R$ y $n \bmod N_R$. Vectoriza por filas con `_mm512_*` pero no register-blockea $C$: paga `load/FMA/store` por cada par $(r, p)$. Solo se activa para hasta $M_R - 1 = 5$ filas residuales por invocacion.
...
Los modulos estan ordenados por familia (`naive`, `loops`, `tiled_ikj*`, `morton*`) y, dentro de cada familia, por nivel de optimizacion (escalar $\to$ AVX-512 $\to$ OpenMP).
...
Tiling explicito $M_c \times K_c$ sobre el orden `ikj`. Apunta al L2 de $1$ MiB por core del EPYC 9R45.

#### Constantes publicas

```c
#define TILED_IKJ_MC_DEFAULT 384u   /* override -DTILED_IKJ_MC_DEFAULT=... */
#define TILED_IKJ_KC_DEFAULT 384u   /* idem */
```

A $M_c = K_c = 384$, los tres paneles activos ($A$: $576$ KiB, $B$: $192$ KiB, $C$: $192$ KiB en el bloque vivo) ocupan $\sim 960$ KiB y caben en el L2 de $1$ MiB del Zen 5. Ambos defaults son `#ifndef`-guarded; el Makefile los sobrescribe via `CFLAGS_O3_ZEN5` (`-DTILED_IKJ_MC_DEFAULT=...`).
...
### 4.4 `matmul_tiled_ikj_avx512`

**Archivos:** [`src/algorithms/tiled_ikj/matmul_tiled_ikj_avx512.h`](../src/algorithms/tiled_ikj/matmul_tiled_ikj_avx512.h), [`src/algorithms/tiled_ikj/matmul_tiled_ikj_avx512.c`](../src/algorithms/tiled_ikj/matmul_tiled_ikj_avx512.c). Usa [`kernel_avx512_tiled.h`](../src/microkernels/kernel_avx512_tiled.h) (Seccion 3.2).
**Estado:** BLIS-style $6 \times 32$ register-blocked sintonizado al Zen 5.

Loop nest Goto-style $p_c \to i_c \to j_r \to i_r$ con microkernel $6 \times 32$ en el centro. $C$ se zera con `memset` al inicio para que cada pasada $p_c$ pueda cargar / acumular / guardar.
...
#define TILED_IKJ_AVX512_MC 288u             /* override -DTILED_IKJ_AVX512_MC=... */
#define TILED_IKJ_AVX512_BS_DEFAULT 256u     /* idem */
extern size_t g_tiled_ikj_avx512_bs;
```

- $M_R = 6$, $N_R = 32$: tile registrado fijo (compile-time).
- $M_C = 288 = 48 \cdot M_R$: tamano del bloque sobre $m$. Panel $A$ activo de $M_C \times k_c$ a $k_c = 256$ ocupa $288$ KiB y cabe holgado en el L2 de $1$ MiB.
- $k_c$ (alias `bs`): default $256$, configurable runtime (ver setter abajo). Panel $B$ activo $k_c \times N_R$ a $k_c = 256$ ocupa $32$ KiB y cabe en el L1d de $48$ KiB con margen para los $12$ acumuladores activos de $C$.

Ambos defaults son `#ifndef`-guarded; el Makefile los sobrescribe en compile-time.

#### `matmul_tiled_ikj_avx512_set_bs`

```c
void matmul_tiled_ikj_avx512_set_bs(size_t bs);
```

Cambia $k_c$ en runtime. Cualquier valor positivo es valido (el microkernel itera $p$ uno a la vez, no requiere $k_c$ multiplo de $16$). Pasar $0$ es invalido y se rechaza.

#### `matmul_tiled_ikj_avx512`
...
**Precondiciones:** mismas que `matmul_naive`. Bordes en $m$ y $n$ que no son multiplos del tile caen al fallback `kernel_avx512_tiled_residual_rows` (Seccion 3.2).

**Compilacion:** requiere `-O3 -march=native` para que `_mm512_fmadd_ps` emita la instruccion FMA-512 real.

#### `benchmark_iterations_tiled_ikj_avx512`
...
**Archivos:** [`src/algorithms/tiled_ikj/matmul_tiled_ikj_omp.h`](../src/algorithms/tiled_ikj/matmul_tiled_ikj_omp.h), [`src/algorithms/tiled_ikj/matmul_tiled_ikj_omp.c`](../src/algorithms/tiled_ikj/matmul_tiled_ikj_omp.c). Usa [`kernel_avx512_tiled.h`](../src/microkernels/kernel_avx512_tiled.h) (Seccion 3.2).
**Estado:** microkernel $6 \times 32$ + `#pragma omp parallel for schedule(static)` en el bucle $i_c$.

Hermano paralelo de `matmul_tiled_ikj_avx512`. Mismo loop nest y mismo microkernel; lo unico que cambia es que el bucle $i_c$ se distribuye entre threads. Cada thread escribe exclusivamente las filas $[i_c, i_c + M_C)$ de $C$, sin conflictos. La region paralela se abre una sola vez por invocacion.
...
#define TILED_IKJ_OMP_MC 288u             /* override -DTILED_IKJ_OMP_MC=... */
#define TILED_IKJ_OMP_BS_DEFAULT 256u     /* idem */
extern size_t g_tiled_ikj_omp_bs;
```

Misma geometria que la version serial; el knob `bs` es independiente para permitir tunear por separado. Defaults `#ifndef`-guarded igual que los de la version serial.
...
Mismo contrato externo que `matmul_tiled_ikj_avx512`. El numero de threads lo fija `OMP_NUM_THREADS` antes de invocar el binario (sin override en codigo). **Compilacion:** requiere ademas `-fopenmp`.
...
**Estado:** kernel recursivo Morton fino, cache-oblivious.
...
`g_recursion_threshold` default $= 32 \cdot 32 \cdot 128 = 131072$ flops elementales. A esa profundidad el panel $A$ activo es $\sim 4$ KiB. Pasar `threshold = 0` al setter imprime warning y mantiene el default.
...
`benchmark_iterations_morton` reorganiza $A$ a Morton **internamente en cada llamada** (el costo entra en el tiempo medido). `benchmark_iterations_morton_preorganized` recibe $A$ ya en Morton y es la que usa `bench_morton_ZEN5` para que la reorganizacion no entre en el tiempo cronometrado.

### 4.7 `matmul_morton_avx512`

**Archivos:** [`src/algorithms/morton/matmul_morton_avx512.h`](../src/algorithms/morton/matmul_morton_avx512.h), [`src/algorithms/morton/matmul_morton_avx512.c`](../src/algorithms/morton/matmul_morton_avx512.c). Usa [`kernel_avx512_morton.h`](../src/microkernels/kernel_avx512_morton.h) (Seccion 3.1).
**Estado:** variante AVX-512 del Morton recursivo sobre Zen 5.
...
Empaqueta $A$ row-major al layout Morton-de-bloques. **Precondiciones:** `m % MORTON_AVX512_TILE == 0` y `is_power_of_two(m / MORTON_AVX512_TILE)`. Aborta si no se cumplen. Complejidad: $O(m^2)$.

#### `matmul_morton_avx512`
...
- $n \geq N_R = 32$. Recomendado $n$ multiplo de $N_R$; trozos no alineados caen a un fallback `ijk` sin vectorizar.
...
`g_recursion_threshold_avx512` default $= 1048576$ flops elementales (panel $A$ activo $\sim 32$ KiB, dos tercios del L1d de $48$ KiB). Separado de `g_recursion_threshold` (Seccion 4.6) porque el leaf AVX-512 amortiza una hoja mucho mayor. Override en compile-time via `-DMORTON_AVX512_THRESHOLD_DEFAULT=...` (Makefile).
...
Misma convencion que en `matmul_morton`: la `_preorganized` es la que usa `bench_morton_avx512_ZEN5`.

### 4.8 `matmul_morton_omp`

**Archivos:** [`src/algorithms/morton/matmul_morton_omp.h`](../src/algorithms/morton/matmul_morton_omp.h), [`src/algorithms/morton/matmul_morton_omp.c`](../src/algorithms/morton/matmul_morton_omp.c). Reusa [`kernel_avx512_morton.h`](../src/microkernels/kernel_avx512_morton.h) (Seccion 3.1) y `reorganize_to_morton_blocks` de la Seccion 4.7.
**Estado:** variante paralela con OpenMP tasks.

Variante paralela de `matmul_morton_avx512`. Misma estructura recursiva y mismo microkernel; agrega `#pragma omp parallel single` en el wrapper publico y emite `omp task` en cada subdivision por encima del threshold de paralelizacion. El scratch buffer del leaf es un pool por-thread indexado por `omp_get_thread_num()`.
...
Mismas precondiciones que `matmul_morton_avx512` (Seccion 4.7). **Compilacion:** requiere ademas `-fopenmp`. El numero de threads lo fija `OMP_NUM_THREADS`.
...
- `g_recursion_threshold_omp`: tamano del sub-problema en que la recursion cae al leaf. Mismo rol que `g_recursion_threshold_avx512`.
...
Override en compile-time via `-DMORTON_OMP_RECURSION_THRESHOLD_DEFAULT=...` y `-DMORTON_OMP_PARALLEL_THRESHOLD_DEFAULT=...`. Las globals son independientes de las de `matmul_morton_avx512`.
...
Misma convencion: la `_preorganized` es la que usa `bench_morton_omp_ZEN5`.
...
```
bench_<variant>_ZEN5 <m> [num_iters] [num_runs]
```

Las variantes con knob de tile-size adicional (`tiled_ikj_avx512`, `tiled_ikj_omp`) aceptan un cuarto argumento:

```
bench_tiled_ikj_avx512_ZEN5 <m> [num_iters] [num_runs] [bs]
bench_tiled_ikj_omp_ZEN5    <m> [num_iters] [num_runs] [bs]
```

La variante `bench_loops_ZEN5` recibe el orden como primer argumento:
...
Los benches de la familia Morton (`bench_morton_ZEN5`, `bench_morton_avx512_ZEN5`, `bench_morton_omp_ZEN5`) **abortan** si $m$ no es potencia de $2$; la reorganizacion a Morton se ejecuta **una sola vez antes del warm-up**, fuera del tiempo cronometrado.
...
```
validate_tiled_ikj_avx512_ZEN5 [m] [bs]
validate_tiled_ikj_omp_ZEN5    [m] [bs]
```
...
- `validate_morton`: 3 invariantes + cross-validation contra `matmul_naive` en $m \in \{4, 16, 64, 256\}$.
- `validate_morton_avx512`, `validate_morton_omp`: 3 invariantes + cross-validation contra `matmul_naive` y contra `matmul_morton`.
- `validate_tiled_ikj{,_avx512,_omp}`: 3 invariantes + cross-validation contra `matmul_naive`.
...
| `validate_tiled_ikj_avx512` | $10^{-4}$ | $10^{-3}$ |
| `validate_tiled_ikj_omp` | $10^{-4}$ | $10^{-3}$ |
| `validate_morton` | $10^{-5}$ | $10^{-4}$ |
| `validate_morton_avx512` | $10^{-4}$ | $10^{-3}$ |
| `validate_morton_omp` | $10^{-4}$ | $10^{-3}$ |

`validate_morton` mantiene tolerancia mas estricta porque el kernel escalar Morton fino no reasocia la suma FP; los demas relajan a $10^{-4} / 10^{-3}$ por el reorden inducido por AVX-512 (`-O3`, intrinsics FMA) y por OpenMP.
...
|---------|-------|----------|
| `naive` | `bin/bench/bench_naive_ZEN5` | `bin/validate/validate_naive_O0` |
| `loops` | `bin/bench/bench_loops_ZEN5` | `bin/validate/validate_loops_O0` |
| `tiled_ikj` | `bin/bench/bench_tiled_ikj_ZEN5` | `bin/validate/validate_tiled_ikj_O0` |
| `tiled_ikj_avx512` | `bin/bench/bench_tiled_ikj_avx512_ZEN5` | `bin/validate/validate_tiled_ikj_avx512_ZEN5` |
| `tiled_ikj_omp` | `bin/bench/bench_tiled_ikj_omp_ZEN5` | `bin/validate/validate_tiled_ikj_omp_ZEN5` |
| `morton` | `bin/bench/bench_morton_ZEN5` | `bin/validate/validate_morton_O0` |
| `morton_avx512` | `bin/bench/bench_morton_avx512_ZEN5` | `bin/validate/validate_morton_avx512_ZEN5` |
| `morton_omp` | `bin/bench/bench_morton_omp_ZEN5` | `bin/validate/validate_morton_omp_ZEN5` |

**Flags de compilacion por sufijo:**

- `_O0`: `-std=c11 -Wall -Wextra -Wpedantic -O0 -g -fno-omit-frame-pointer -D_POSIX_C_SOURCE=200809L`.
- `_ZEN5`: `-O3 -march=native` mas los `-D...` de los thresholds del Makefile (`CFLAGS_O3_ZEN5`).
- `_ZEN5` para variantes `_omp` ademas con `-fopenmp` (`CFLAGS_OMP_ZEN5`).
...
| `bin/tests/test_kernel_avx512_morton` | Microkernel $4 \times 32$ de [`kernel_avx512_morton.h`](../src/microkernels/kernel_avx512_morton.h) |
| `bin/tests/test_kernel_avx512_tiled` | Microkernel $6 \times 32$ de [`kernel_avx512_tiled.h`](../src/microkernels/kernel_avx512_tiled.h) |
