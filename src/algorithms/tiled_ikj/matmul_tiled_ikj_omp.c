...
/*
matmul_tiled_ikj_omp: C = A * B con A(m x k), B(k x n), C(m x n) con paralelización AVX-512.
INPUTS:
...
NOTA: EPYC 9R45 tiene 8 cores físicos (SMT deshabilitado por hypervisor).
Usar OMP_NUM_THREADS=8 OMP_PLACES=cores OMP_PROC_BIND=close.
Misma estructura de matmul_tiled_ikj_avx512.c
*/
                      ...
                      size_t m, size_t k_dim, size_t n)
{
    ...
    #pragma omp parallel
    {
        ...
        for (size_t kk = 0; kk < k_dim; kk += KC) {
            ...
            // Omp distribuye los threads (8 en este caso) para cada ic.
                                    ...
                                    size_t num_iters)
{
    ...
    */
    ...
    for (size_t iter = 0; iter < num_iters; ++iter) {
        ...
        - Cada B_out_{i} es contiguo. (Ver cómo se define el buffer completo en bench_tiled_ikj_omp.c)
        */
