 ...
 *
 * The 6x16 microkernel and the AVX2 residual-rows fallback live in
 * kernel_avx2_tiled.h and are exposed as `static inline` so each
 * translation unit (this file and matmul_tiled_ikj_avx2.c) gets its own
 * inlined copy under -O3 without crossing a function-call boundary.
 * Earlier revisions duplicated the kernel body in each .c for the same
 * reason; the shared header version preserves the same codegen as long
 * as `static inline` is honored (verified by objdump on the resulting
 * binaries).
 *
