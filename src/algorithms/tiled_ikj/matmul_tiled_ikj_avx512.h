/*
 * matmul_tiled_ikj_avx512.h - BLIS-style 6x32 register-blocked matmul
 *                      with AVX-512 + FMA, tuned for AMD EPYC 9R45
 *                      (Zen 5) on AWS c8a.2xlarge.
 *
 * Renamed from matmul_tiled_ikj_avx2.h: the leaf uses the 6x32
 * AVX-512 microkernel from kernel_avx512_tiled.h. A 6x32 sub-tile of
 * C lives in 12 ZMM accumulators (6 rows x 2 ZMMs of 16 lanes) during
 * the entire kc sweep, so C touches memory only twice per micro-tile
 * (load on entry, store on exit). Layout of the architectural ZMMs:
 *
 ...
 *   pc loop   step kc  (== g_tiled_ikj_avx512_bs, configurable, default 256)
 *     ic loop step mc  (= 288, fixed in source / -DTILED_IKJ_AVX512_MC)
 ...
 *
 * Backwards-compat CLI: `[bs]` is still accepted by bench_tiled_ikj_avx512,
 * which sets g_tiled_ikj_avx512_bs (interpreted as kc, the inner panel
 * depth). mc / mr / nr are fixed in the source because their optimal
 * values are dictated by the register file geometry and not by tuning.
 * Validators that call matmul_tiled_ikj_avx512_set_bs() with arbitrary
 * values still work; the kernel handles any kc >= 1.
 */
 ...
 * may override with -DTILED_IKJ_AVX512_BS_DEFAULT=... for sweeps. */
...
/*
 * matmul_tiled_ikj_avx512: compute C = A * B with BLIS-style register blocking.
 *
