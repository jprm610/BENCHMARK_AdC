/*
 * test_kernel_avx512_tiled.c - Unit test for the 6x32 AVX-512 microkernel
 * declared in kernel_avx512_tiled.h (header-only static inline). Mirrors
 * test_kernel_avx512_morton.c, which covers the sibling 4x32 microkernel
 * used by the Morton family.
 *
 * The 6x32 tile is used by matmul_tiled_ikj_avx512 and matmul_tiled_ikj_omp.
 * It holds 12 ZMM accumulators (6 rows x 2 vectors of 16 floats each),
 * leaving 20 of the 32 architectural ZMM registers free for the two B
 ...
 *
 *   Test 1: kc=1, A=[1,1,1,1,1,1]^T, B=known row of 32 distinct values.
 ...
 *           matmul_tiled_ikj_avx512 (default bs is 256, but 128 is a
...
#include "kernel_avx512_tiled.h"
#include "matrix_utils.h"

#define MR ((size_t)KERNEL_AVX512_TILED_MR)  /* 6  */
#define NR ((size_t)KERNEL_AVX512_TILED_NR)  /* 32 */

static int g_failed = 0;

/* Reference matmul that accumulates the product in double precision and
 * stores the result back as scalar_t. This is the ground truth the AVX-512
...
static void test_kc_1(void)
{
    ...
    kernel_avx512_tiled_6x32(C, ldc, A, lda, B, ldb, kc);
...
int main(void)
{
    printf("=== kernel_avx512_tiled_6x32 unit tests ===\n");
