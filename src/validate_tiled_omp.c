/*
 * validate_tiled_omp.c - Algebraic sanity checks for matmul_omp.
 *
 * Same four tests as validate_tiled_avx2.c:
 *   1. A * 0 == 0
 ...
 *
 * Usage:
 *   validate_tiled_omp_O3 [m] [bs]
 *     m  : problem size, default 256
 *     bs : block size passed to matmul_tiled_omp_set_bs, default 64
 *
...
{
    size_t m  = 256;
    size_t bs = TILED_OMP_BS_DEFAULT;
    ...
    matmul_tiled_omp_set_bs(bs);

    size_t n = m;
    size_t k = m;

    printf("Validating matmul_omp at m=%llu (bs=%llu)\n",
           (unsigned long long)m, (unsigned long long)g_tiled_omp_bs);
    ...
    matmul_tiled_omp(C, A, zero, m, k, n);
    all_ok &= check("A * 0 == 0", expected_zero, C, m * n);
    xfree(zero);
    xfree(expected_zero);

    /* Test 2: I * Z == Z */
    matmul_tiled_omp(C, I_m, Z, m, k, n);
    all_ok &= check("I * Z == Z", Z, C, m * n);

    /* Test 3: A * (Z1 + Z2) == A*Z1 + A*Z2 */
    matmul_tiled_omp(C,    A, Zsum, m, k, n);
    matmul_tiled_omp(C2,   A, Z1,   m, k, n);
    matmul_tiled_omp(Ctmp, A, Z2,   m, k, n);
