 ...
 *     bs : block size passed to matmul_omp_set_bs, default 64
 *
...
int main(int argc, char **argv)
{
    ...
    matmul_omp_set_bs(bs);

    size_t n = m;
    size_t k = m;

    printf("Validating matmul_omp at m=%llu (bs=%llu)\n",
           (unsigned long long)m, (unsigned long long)g_omp_bs);
    ...
    matmul_omp(C, A, zero, m, k, n);
    all_ok &= check("A * 0 == 0", expected_zero, C, m * n);
    xfree(zero);
    xfree(expected_zero);

    /* Test 2: I * Z == Z */
    matmul_omp(C, I_m, Z, m, k, n);
    all_ok &= check("I * Z == Z", Z, C, m * n);

    /* Test 3: A * (Z1 + Z2) == A*Z1 + A*Z2 */
    matmul_omp(C,    A, Zsum, m, k, n);
    matmul_omp(C2,   A, Z1,   m, k, n);
    matmul_omp(Ctmp, A, Z2,   m, k, n);
