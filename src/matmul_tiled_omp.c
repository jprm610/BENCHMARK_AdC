/*
 * matmul_tiled_omp.c - 6-loop tiled matmul with AVX2+FMA and OpenMP parallelization.
 *
...
size_t g_tiled_omp_bs = TILED_OMP_BS_DEFAULT;

void matmul_tiled_omp_set_bs(size_t bs)
{
    if (bs == 0 || (bs % 8) != 0) {
        fprintf(stderr,
                "Warning: matmul_tiled_omp_set_bs(%llu) ignored; "
...
void matmul_tiled_omp(scalar_t *C,
                      const scalar_t *A,
                      const scalar_t *B,
                      size_t m, size_t k, size_t n)
{
    const size_t BS = g_tiled_omp_bs;
...
void benchmark_iterations_tiled_omp(scalar_t *B_out,
                                    const scalar_t *A,
                                    const scalar_t *Z,
                                    size_t m, size_t n,
                                    size_t num_iters)
{
    ...
    for (size_t iter = 0; iter < num_iters; ++iter) {
        matmul_tiled_omp(B_next, A, B_curr, m, m, n);
